"""
SENTINEL — Redesigned Master PowerPoint Generator (14-Slide Spacious Edition)
============================================================================
Implements:
  - 14 spacious slides with real diagrams, decision trees, and zone maps
  - Slide 12 expanded into a panoramic 3-Stage End-to-End Architecture Blueprint
  - Removed slides 13 (Demo Timeline) and 14 (Verification Table) as requested
  - Slide 13 is now 'Why SENTINEL' and Slide 14 is 'Conclusion'
  - All unsupported claims removed; terminology strictly accurate
  - Speaker notes embedded on every slide
"""
import os
import sys
...
TXT_WHITE  = RGBColor(248,250,252)      # #F8FAFC
TXT_MUTED  = RGBColor(148,163,184)      # #94A3B8
...
BLUE_LT    = RGBColor(96, 165,250)      # #60A5FA
BOX_FILL   = RGBColor(15, 23, 42)       # #0F172A
...
# ══════════════════════════════════════════════════════════════
# SLIDE 12 — FULL SPACIOUS 3-STAGE END-TO-END ARCHITECTURE
# (Expanded to use full widescreen canvas with generous breathing room)
# ══════════════════════════════════════════════════════════════
def slide_12(prs):
    sl = prs.slides.add_slide(prs.slide_layouts[6])
    _set_bg(sl)
    _title(sl, "Full End-to-End System Architecture Blueprint",
           "Three distinct operational stages: Ingestion & Privacy Boundary -> Intelligence & Memory -> Decision & Controlled Defense")

    # Layout: 3 wide columns across the 13.33 inch slide with full vertical room
    col_w = 3.65
    c1_x = 0.5
    c2_x = 4.85
    c3_x = 9.2

    # Background Zone Panels
    _rect(sl, c1_x - 0.1, 1.35, col_w + 0.2, 4.85, ZONE_LOCAL, border=CYAN, bw=Pt(1.5))
    _rect(sl, c2_x - 0.1, 1.35, col_w + 0.2, 4.85, RGBColor(7, 18, 38), border=BLUE_LT, bw=Pt(1.5))
    _rect(sl, c3_x - 0.1, 1.35, col_w + 0.2, 4.85, ZONE_CLOUD, border=YELLOW, bw=Pt(1.5))

    # STAGE 1: INGESTION & PRIVACY
    _tb(sl, c1_x, 1.45, col_w, 0.35, "STAGE 1: Ingestion & Privacy", color=CYAN, size=11, bold=True, align=PP_ALIGN.CENTER)
    
    _mbox(sl, c1_x, 1.85, col_w, 1.15,
          [("1. Wazuh / SIEM Telemetry Ingest", 11, TXT_WHITE, True, PP_ALIGN.LEFT),
           ("src/ingestion/wazuh_listener.py", 9, CYAN, False, PP_ALIGN.LEFT),
           ("Async FastAPI HTTP Syslog Webhook", 9, TXT_MUTED, False, PP_ALIGN.LEFT)],
          fill=BOX_DARK2, border=DIVIDER)
    
    _arr(sl, c1_x + col_w/2 - 0.25, 3.0, 0.5, 0.35, "down", TXT_MUTED, 16)

    _mbox(sl, c1_x, 3.35, col_w, 1.25,
          [("2. Zero-Trust Data Sanitizer", 11, GREEN, True, PP_ALIGN.LEFT),
           ("src/sanitizer.py", 9, CYAN, False, PP_ALIGN.LEFT),
           ("PII -> Synthetic Token Mapping ([USER_1])", 9, TXT_WHITE, False, PP_ALIGN.LEFT),
           ("Prompt Injection Threat Firewall", 9, YELLOW, False, PP_ALIGN.LEFT)],
          fill=RGBColor(8, 25, 14), border=GREEN, bw=Pt(1.5))

    _arr(sl, c1_x + col_w/2 - 0.25, 4.6, 0.5, 0.35, "down", TXT_MUTED, 16)

    _mbox(sl, c1_x, 4.95, col_w, 1.05,
          [("3. Local RAM Identity Map", 11, TXT_WHITE, True, PP_ALIGN.LEFT),
           ("Reversible lookup table in RAM memory", 9, TXT_MUTED, False, PP_ALIGN.LEFT),
           ("Key NEVER leaves local workstation", 9, RED, True, PP_ALIGN.LEFT)],
          fill=BOX_DARK2, border=DIVIDER)

    # Connector Arrow 1 -> 2
    _tb(sl, 4.35, 3.5, 0.45, 0.4, "->", color=CYAN, size=24, bold=True, align=PP_ALIGN.CENTER)

    # STAGE 2: THREAT CONTEXT & GRAPH
    _tb(sl, c2_x, 1.45, col_w, 0.35, "STAGE 2: Intelligence & Graph", color=BLUE_LT, size=11, bold=True, align=PP_ALIGN.CENTER)

    _mbox(sl, c2_x, 1.85, col_w, 1.15,
          [("4. MITRE ATT&CK Taxonomy Mapper", 11, TXT_WHITE, True, PP_ALIGN.LEFT),
           ("src/mitre_mapper.py", 9, BLUE_LT, False, PP_ALIGN.LEFT),
           ("Tags Tactic (e.g. Credential Access)", 9, TXT_MUTED, False, PP_ALIGN.LEFT),
           ("Tags Technique ID (e.g. T1110 Brute Force)", 9, TXT_MUTED, False, PP_ALIGN.LEFT)],
          fill=BOX_DARK2, border=DIVIDER)

    _arr(sl, c2_x + col_w/2 - 0.25, 3.0, 0.5, 0.35, "down", TXT_MUTED, 16)

    _mbox(sl, c2_x, 3.35, col_w, 1.25,
          [("5. ChromaDB RAG Vector Threat Memory", 11, BLUE_LT, True, PP_ALIGN.LEFT),
           ("src/memory.py", 9, BLUE_LT, False, PP_ALIGN.LEFT),
           ("Semantic similarity search across cases", 9, TXT_WHITE, False, PP_ALIGN.LEFT),
           ("Enriches prompt with historical context", 9, TXT_MUTED, False, PP_ALIGN.LEFT)],
          fill=RGBColor(10, 22, 45), border=BLUE_LT, bw=Pt(1.5))

    _arr(sl, c2_x + col_w/2 - 0.25, 4.6, 0.5, 0.35, "down", TXT_MUTED, 16)

    _mbox(sl, c2_x, 4.95, col_w, 1.05,
          [("6. Incident Correlator & Attack Graph", 11, YELLOW, True, PP_ALIGN.LEFT),
           ("src/correlation/attack_graph.py", 9, BLUE_LT, False, PP_ALIGN.LEFT),
           ("Links User <-> Host <-> IP <-> Technique", 9, TXT_MUTED, False, PP_ALIGN.LEFT)],
          fill=BOX_DARK2, border=DIVIDER)

    # Connector Arrow 2 -> 3
    _tb(sl, 8.7, 3.5, 0.45, 0.4, "->", color=YELLOW, size=24, bold=True, align=PP_ALIGN.CENTER)

    # STAGE 3: AI DECISION & ACTIVE DEFENSE
    _tb(sl, c3_x, 1.45, col_w, 0.35, "STAGE 3: Decision & Response", color=YELLOW, size=11, bold=True, align=PP_ALIGN.CENTER)

    _mbox(sl, c3_x, 1.85, col_w, 1.15,
          [("7. 3-Tier AI Routing Cascade", 11, GREEN, True, PP_ALIGN.LEFT),
           ("src/router.py", 9, YELLOW, False, PP_ALIGN.LEFT),
           ("Tier 1: Local GPU ($0.00) -> Tier 2: Groq/Gemini", 9, TXT_WHITE, False, PP_ALIGN.LEFT),
           ("Tier 3: OpenRouter Nemotron 550B model", 9, TXT_MUTED, False, PP_ALIGN.LEFT)],
          fill=RGBColor(8, 25, 12), border=GREEN, bw=Pt(1.5))

    _arr(sl, c3_x + col_w/2 - 0.25, 3.0, 0.5, 0.35, "down", TXT_MUTED, 16)

    _mbox(sl, c3_x, 3.35, col_w, 1.25,
          [("8. Human-in-the-Loop (HITL) Gate", 11, YELLOW, True, PP_ALIGN.LEFT),
           ("src/api/main.py  |  src/sandbox.py", 9, YELLOW, False, PP_ALIGN.LEFT),
           ("AST Code Sandbox blocks malicious shell calls", 9, TXT_WHITE, False, PP_ALIGN.LEFT),
           ("Officer explicit APPROVE / REJECT required", 9, YELLOW, False, PP_ALIGN.LEFT)],
          fill=RGBColor(25, 20, 5), border=YELLOW, bw=Pt(1.5))

    _arr(sl, c3_x + col_w/2 - 0.25, 4.6, 0.5, 0.35, "down", TXT_MUTED, 16)

    _mbox(sl, c3_x, 4.95, col_w, 1.05,
          [("9. Controlled Defense & Courtroom PDF", 11, TXT_WHITE, True, PP_ALIGN.LEFT),
           ("src/response/ [MOCK]  |  src/reports/", 9, YELLOW, False, PP_ALIGN.LEFT),
           ("Append-only audit trail + Executive PDF report", 9, TXT_MUTED, False, PP_ALIGN.LEFT)],
          fill=BOX_DARK2, border=DIVIDER)

    # Bottom Full-Width Takeaway Bar
    _rect(sl, 0.4, 6.35, 12.53, 0.65, fill=RGBColor(10, 20, 42), border=CYAN, bw=Pt(1.5))
    _tb(sl, 0.6, 6.4, 12.1, 0.55,
        "🔒 ARCHITECTURAL GUARANTEE: Raw telemetry remains in Stage 1 Local Trust Zone. Cloud models only receive sanitized dummy tokens. No autonomous destructive actions without Officer Approval.",
        color=YELLOW, size=10, bold=True, align=PP_ALIGN.CENTER)

    _notes(sl, "This is the complete, master architecture of SENTINEL organized into three clear, spacious stages. In Stage 1, raw telemetry enters through the Wazuh listener and is immediately scrubbed by the Zero-Trust Data Sanitizer; raw identifiers remain inside local RAM and are never transmitted. In Stage 2, MITRE taxonomy mapping, ChromaDB RAG memory search, and graph correlation enrich the threat context. In Stage 3, the 3-Tier AI router processes the alert (starting locally on GPU for $0 cost), passes through the AST sandbox and HITL approval gateway, and finishes with an immutable audit log and courtroom PDF report.")


# ══════════════════════════════════════════════════════════════
# SLIDE 13 — WHY SENTINEL (3 Defensible Differentiators)
# ══════════════════════════════════════════════════════════════
def slide_13(prs):
    sl = prs.slides.add_slide(prs.slide_layouts[6])
    _set_bg(sl)
...
# MAIN
# ══════════════════════════════════════════════════════════════
def build(output=r"C:\Users\siva2\Desktop\SENTINEL_Master_Pitch_Deck_2026.pptx"):
    prs = Presentation()
    ...
    print(f"Building redesigned SENTINEL deck ({len(builders)} slides)...")
    ...
    try:
        prs.save(output)
        print(f"\n🎉 Saved successfully to: {output}")
    except PermissionError:
        fallback = r"C:\Users\siva2\Desktop\SENTINEL_Spacious_Pitch_Deck_v3.pptx"
        prs.save(fallback)
        print(f"\n⚠️  Original file was locked in PowerPoint. Saved successfully to: {fallback}")
        output = fallback
