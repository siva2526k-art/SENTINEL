...
def get_pristine_doc(input_path):
    doc = fitz.open(input_path)
    # Redact any previously added content blocks to keep the raw Sairam template header & border
    for page in doc:
        blocks = page.get_text('blocks')
        for b in blocks:
            text = b[4]
            # Preserve template headers and section prompts
            if any(k in text for k in ['STUDENT ACTIVITY FILE', 'INNOVATIVE DESIGN LAB', 'WEEK – 2', 'WEEK - 2', '2. Problem Identification', 'Identify real-world', 'Skills, experiences', 'Prepare a Passion CV']):
                continue
            # Redact added user content
            if text.strip():
                page.add_redact_annot(fitz.Rect(b[0]-2, b[1]-2, b[2]+2, b[3]+2))
...
def fill_week2_pdf():
    ...
    doc = get_pristine_doc(input_path)
    dark = (0.05, 0.1, 0.25)
    font_name = "helv"
    ...
    p1.insert_textbox(fitz.Rect(95, 235, 755, 520), w2_p1_text, fontsize=9.2, fontname=font_name, color=dark)
    ...
    p2.insert_textbox(fitz.Rect(95, 120, 755, 510), w2_p2_text, fontsize=9.2, fontname=font_name, color=dark)
    ...
    w2_p3_text = (
        ...
        "  3. Research & Academic Output: Co-author of academic literature survey analyzing 6 peer-reviewed IEEE papers on threat triage and privacy-preserving AI.\n\n"
    ...
    p3.insert_textbox(fitz.Rect(95, 160, 755, 520), w2_p3_text, fontsize=9.0, fontname=font_name, color=dark)

    doc.save(output_path)
    shutil.copy2(output_path, r"C:\Users\siva2\OneDrive\Desktop\IDL WEEK 2.pdf")
    shutil.copy2(output_path, sentinel_doc_path)
    doc.close()
    print(f"✅ Pristine Clean Week 2 PDF Generated: {output_path}")
    return output_path
